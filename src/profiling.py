...
alphasliders = {m:[FloatSlider(description=l, min=0., max=1., step=0.01, value=np.random.rand()) for l in CHANNEL_NAMES] for m in AGENT_NAMES}
...
METHODS = ['v', 'ep', 'ei']
STEPS = 500
DATASET = 'datasets/coco_animals_validation_bird.csv'
for i, (input_sample, ground_truth) in enumerate(SegmentationModel('dummy').load_dataset(DATASET, batch_size=1)):
    ...
    for step, (status, curr_agreement, curr_proposals) in next_step_v:
        #result_logger.log_step(sample=i, step=step, method='v', status=status, agreement=curr_agreement, proposals=curr_proposals, ground_truth=ground_truth_np)
    ...
    for step, (status, curr_agreement, curr_proposals) in next_step_ep:
        #result_logger.log_step(sample=i, step=step, method='ep', status=status, agreement=curr_agreement, proposals=curr_proposals, ground_truth=ground_truth_np)
    ...
    for step, (status, curr_agreement, curr_proposals) in next_step_ei:
        #result_logger.log_step(sample=i, step=step, method='ei', status=status, agreement=curr_agreement, proposals=curr_proposals, ground_truth=ground_truth_np)
